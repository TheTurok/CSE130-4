...
let lookup (x,evn) = 
  match listAssoc(x, evn) with
    | Some x -> x
    | None -> raise (MLFailure ("varibale not bound: " ^ x));;
        ...
        (match (eval(evn,e1),eval(evn, e2)) with 
          | (Int e1, Int e2) ->
              (match b with
                ...
                | Lt -> Bool(e1 < e2)
                | Le -> Bool(e1 <= e2)
                | _ -> raise (MLFailure("Invalid Operation: " ^ exprToString e))
              )
          | (Bool e1, Bool e2) ->
              (match b with
                | Eq -> Bool(e1 = e2)
          ...
          | (Int e1, Nil) ->
              (match b with 
                | Cons -> Pair (Int e1, Nil)
                | _ -> raise (MLFailure("Invalid Operation: " ^ exprToString e))
              )
          | (Int e1, Pair(a, c)) ->
              (match b with
                | Cons -> Pair (Int e1, (eval(evn, e2)))
                | _ -> raise (MLFailure("Invalid Operation: " ^ exprToString e))
              )
          | _ -> raise (MLFailure("Invalid Operation: " ^ exprToString e))
        )
    | If (i,t,f) -> 
        (match eval(evn, i) with
          | Bool true -> eval(evn, t)
          | Bool false -> eval(evn, f)
          | _ -> raise (MLFailure("Invalid Type: " ^ exprToString i))
        )
    | Let (l, e1 , e2) -> eval((l, eval(evn, e1))::evn, e2)
    | Letrec (lr, e1 , e2) -> let v = eval (evn, e1) in
        let environment = 
          (match v with
            | Closure (cevn, None, x, e) -> Closure (cevn, Some lr, x, e)
            | _ -> v) in
        let evn2 = (lr, environment)::evn in
          eval (evn2, e2)
    | App(a, e1)  -> 
        (match a with
          | Var "hd" -> (match eval(evn, e1) with Pair(e1, e2) -> e1)
          | Var "tl" -> (match eval(evn, e1) with Pair(e1, e2) -> e2)
          | _ -> (let Closure (evn2, f, x, e2) = eval(evn, a) in
                  let environment = 
                    (match f with
                      | Some func -> (func, Closure (evn2, f, x, e2))::(x, eval(evn, e1))::evn2
                      | None -> (x, eval(evn, e1))::evn2) in 
                    eval(environment, e2))
        )
    | Fun (f, e1) -> Closure(evn, None, f, e1)
    | _ -> raise (MLFailure("Invalid Operation: " ^ exprToString e))
